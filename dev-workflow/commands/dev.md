...
## 阶段小结规则（Proactive Summarization）

> **目标**：在上下文快满前主动小结，不依赖用户喊「继续」。

### 触发条件

| 条件 | 阈值 | 动作 |
|------|--------|------|
| 工具调用次数 > 20 | 预警 | 小结当前进度，建议压缩 |
| 工具调用次数 > 35 | 强制 | 强制写入记忆，提示用户输入 `/compact` |
| Phase 结束 | 每个 Phase 完成时 | 追加小结到 `memory/YYYY-MM-DD.md` |

### 小结模板

每个 Phase 结束时，必须输出以下格式的小结：
...
| Phase 3 | skill-dev-design | 详细设计文档生成 |
| Phase 4 | skill-dev-writing-plans | 实现计划拆解（2-5 分钟粒度） |
| Phase 4 | skill-dev-subagent | 子 Agent 驱动开发（并行执行任务） |
...
- **Phase 4 前** → 调用 `skill-dev-writing-plans` 检查是否需要拆解任务
- **Phase 5 前** → 调用 `skill-dev-test` 确保 TDD 流程
- **Phase 6 前** → 调用 `skill-dev-review` 准备两阶段审查
- **完成声明前** → 调用 `skill-dev-verification` 执行 5 步验证
